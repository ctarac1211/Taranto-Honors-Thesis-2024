library(readxl)
MDA_ELISA_Geratric_data_analysis <- read_excel("MDA ELISA Geratric data analysis.xlsx")
library(ggplot2)
library(ggsignif)

attach(MDA_ELISA_Geratric_data_analysis)

wt <- `MDA Concentration (ug/mL)`[Genotype == "Wild-type"]
kcnh2a <- `MDA Concentration (ug/mL)`[Genotype == "kcnh2a"]
...
new_MDA <- MDA_ELISA_Geratric_data_analysis
...
plot <-ggplot(new_MDA, aes(x=Genotype, y=`MDA Concentration (ug/mL)`, color = Genotype)) +
  ...
  ylab("Relative MDA Protein Adduct Concentration (ug/mL)")
